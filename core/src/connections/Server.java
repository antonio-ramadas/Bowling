...
public class Server {
	...
	private Boolean player1_isConnected = false;
	private Boolean player2_isConnected = false;
	...
	private DataPacket Data1;
	private DataPacket Data2;
	...
	{
		if (PlayerNumber == 1 && connect_Player1.isAlive() == false)
		{
			connect_Player1 = new Thread(new connectPlayer1());
			connect_Player1.start();
			return true;
		}
		if (PlayerNumber == 2 && connect_Player2.isAlive() == false)
		{
	...
	public DataPacket getLatestDataPlayers(int PlayerNumber){
		if(PlayerNumber == 1 && receive_Player1.isAlive() == true && player1_isConnected == true)
			return Data1;
		if(PlayerNumber == 2 && receive_Player2.isAlive() == true && player2_isConnected == true)
			return Data2;
	...
	private class connectPlayer2 implements Runnable
	{
		public void run() {
			...
			try {
				...
				outputToPlayer2.println(Integer.toString(1));
	...
	private class receivePlayer1 implements Runnable
	{
		public void run() {
			...
			{
				try {
					if (inputFromPlayer1.ready())
					{
						//Freedom Version
						messagePlayer1 = inputFromPlayer1.readLine();
						while(!inputFromPlayer1.ready() && player1_isConnected)
							;
